...
def main() -> int:
    ...
    sc = mcfg["decode"]["self_consistency"]  # n=11 PAPER-SPECIFIED
    ...
    need = [t for t in targets
            if not split_is_complete("self_consistency", *t, limit=args.limit)]
    ...
    (paths.RESULTS / "phase12_mc_results.json").write_text(
        json.dumps({"strategy": "self_consistency", "n_decodes": sc["n"],
                    "model": usage["model"], "mock": args.mock, "seed": seed,
